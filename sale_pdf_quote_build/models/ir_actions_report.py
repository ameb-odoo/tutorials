...
_original_render_qweb_pdf_prepare_streams = (
    IrActionsReport._render_qweb_pdf_prepare_streams
)


def _reordered_render_qweb_pdf_prepare_streams(self, report_ref, data, res_ids=None):
    """Override to add and fill headers, footers and product documents to the sale quotation."""
    # region - check if the pdf_reordered checkbox is true or not and act accordingly
    orders = self.env["sale.order"].browse(res_ids)

    # Check for each order if the pdf_reordered is True
    orders_to_regenerate = [order.id for order in orders if not order.pdf_reordered]

    if orders_to_regenerate:
        return _original_render_qweb_pdf_prepare_streams(
            self, report_ref, data, res_ids=orders_to_regenerate
        )
    # endregion
    ...
    for order in orders:
        ...
        if initial_stream:
            ...
            # region - Changing order of prodcut document and the order itself
...
IrActionsReport._render_qweb_pdf_prepare_streams = (
    _reordered_render_qweb_pdf_prepare_streams
)
